...
For example, take a random anchor set, as shown in Figure~\ref{fig:sampleProcrustes1}.  The figure shows the three triangles, which are the local anchor coordinates, the actual global coordinates, and the predicted coordinates after applying the calculated transformation from local anchor coordinates to actual anchor coordinates.  This demonstrates that the transformation calculated by Procrustes is not perfect, as the actual and calculated coordinates do not align exactly.
