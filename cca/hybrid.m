%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Hybird localization solution
%  
%  MDS solver written by Yi Shang and modified by Ying Zhang
%  CCA added by Li Li
%
% function t = hybrid (t)
% 
...
%         refinement for now
        map{ii} = Y(:,1:2);
        local_network_c=cca(Dhop,map{ii},100,Dhop);
        map{ii}=local_network_c;
%         local_size=size(map{ii});
%         D_C = sqrt(disteusq(map{ii},map{ii},'x'));
% %                  D_dist_mean = mean((mean(abs(D_C-local_distance_deployed)))');
%             D_dist_mean = mean((mean(abs(D_C-Dhop)))');
%             D_dist_mean=D_dist_mean/r; 
%             D_error_mean_compute=D_dist_mean;
%         
%         for kk=1:3 %run 50 more cycles each time to see if we get any better       
%             local_network_c=cca(Dhop,map{ii},50,Dhop);
%             D_C = sqrt(disteusq(local_network_c,local_network_c,'x'));
%             D_dist_mean = mean((mean(abs(D_C-Dhop)))');
%             D_dist_mean=D_dist_mean/r; 
%                    if((D_dist_mean<D_error_mean_compute))
%                        D_error_mean_compute=D_dist_mean;
%                        map{ii}=local_network_c;
%                    end
