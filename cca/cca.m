...
%%%%%%%%%%%%%%can use the following block to generate sample_inds
% sample_inds=[randperm(noc)'];
% sample_inds=repmat(sample_inds(:,[1:1]),epochs,1); 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% calculate error
% next two lines changed by Li from 'error' to 'err'
%error = cca_error(P,Mdist,lambda(train_len)); %commented out by li, not
%used.
% err =cca_error(P,Mdist,lambda(train_len)); %commented out by li, useful
% fprintf(2,'%d iterations, error %f          \n', epochs, err); %commneted out by li, useful

% set projections of totally unknown vectors as unknown
% unknown = find(sum(isnan(D)')==dim);  commented out by li
% P(unknown,:) = NaN;  commented out by li
