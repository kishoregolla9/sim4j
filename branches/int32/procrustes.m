...
%   $Revision: 1.2.2.4 $  $Date: 2008/05/12 21:38:17 $

pnames = {   'scaling'  'reflection'};
dflts =  {       true         'best'};
[eid,errmsg,doScaling,doReflection] = statgetargs(pnames, dflts, varargin{:});
if ~isempty(eid)
...
ssqX = sum(X0.^2,1);
ssqY = sum(Y0.^2,1);
constX = 0; % all(ssqX <= abs(eps(class(X))*n*muX).^2);
constY = 0; % all(ssqY <= abs(eps(class(X))*n*muY).^2);
ssqX = sum(ssqX);
ssqY = sum(ssqY);
...
if ~constX & ~constY
    ...
    if doScaling
        ...
        if nargout > 1
            Z = normX*traceTA * Y0 * T + double(repmat(muX, n, 1));
        end
    else % if ~doScaling
        b = 1;
