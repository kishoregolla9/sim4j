function [node]=localMapConnectivityOnlyGrid(Network,epochs,r)
%Computes the node coordinates of each node in the local map for every node in the 'Network'.
%This function intends to improve the grid network position accuracy in
%certain scenarios using range free method: e.g., loop grid and rectangle grid. 
% It applies the LEM to establish the local edge hop approximation. This function applies two
%levels of LEM. If wants to apply only one level of LEM, use localMapConnectivityOnlyGrid1.
%For grid loop network, my try r1=1.25 and r2=1.75. For rectangle grid
%network may try r1=1.4 and r2=1.75. 
% Network - input deployed network
% epochs - required number of training cycles (e.g., 100)
% r -radius
...
r1=1.25;  %deployment unit radius disk setup
for i=1:N
    for j=1:N
        if D(i,j)<r1
            D_hopCount1(i,j)=1;
...
r2=1.6;  %deployment unit diagonal radius disk setup
for i=1:N
    for j=1:N
        if D(i,j)<r2
            D_hopCount2(i,j)=1;
...
%localDist finds out who are the neighbors within "hop" distance and builds the local map
%distance matrix.
